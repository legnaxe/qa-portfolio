...
1. Функциональное тестирование (SauceDemo, Работа России)
2. Работа с SQL (FTB)
...
#### BUG_002: При входе через Госуслуги отображается техническая ошибка вместо редиректа

**Окружение:**
- OS: Windows 10
- Браузер: Google Chrome, версия: 149.0.7778.218

**Priority:** High
**Severity:** Major

**Заголовок:** Страница авторизации: при нажатии на "Войти через Госуслуги" отображается JSON-ошибка вместо перехода на портал

**Предусловие:** Открыта страница авторизации `https://trudvsem.ru/auth/login`

**Шаги воспроизведения:**
1. Нажать кнопку **"Войти через портал "Госуслуги""**

**Фактический результат:**  
Вместо редиректа на портал Госуслуг пользователь видит на экране техническое сообщение:
```json
{"code":"SERVER_ERROR","comment":"exception, look log file","details":{}}
```

**Ожидаемый результат:**
При нажатии на кнопку система должна перенаправить пользователя на сайт Госуслуг (esia.gosuslugi.ru) для авторизации. В случае ошибки на сервере пользователь должен видеть понятное сообщение, например: "Сервис временно недоступен. Попробуйте позже".

**Обоснование:**
Техническая информация об ошибке (упоминание "log file") не предназначена для пользователей: она может запутать пользователя и раскрывает внутреннее устройство системы, что небезопасно.

**Скриншот:**

![Ошибка при входе через Госуслуги](screenshots/BUG_002_esia_error.JPG)
